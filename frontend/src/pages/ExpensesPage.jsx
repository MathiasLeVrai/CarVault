...
const ChartTip = ({ active, payload, label }) => { if(!active||!payload?.length) return null; return <div className="bg-white border-2 border-ink rounded-xl px-4 py-3 shadow-[3px_3px_0_#1a1a1a]"><p className="text-xs font-bold text-ink-light">{label}</p><p className="text-sm font-black text-ink">{formatCurrency(payload[0].value)}</p></div>; };
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between animate-slide-up">
        <div>
          <h1 className="text-2xl font-black text-ink">Dépenses</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5 stagger">
        ...
        <StatCard icon={Calculator} label="Moyenne mensuelle" value={formatCurrency(avg)} color="lime" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        <Card className="lg:col-span-3 animate-slide-up" style={{animationDelay:'80ms'}}>
          <div className="flex items-center justify-between mb-5">
            <h3 className="text-base font-black text-ink">Évolution</h3>
          ...
          <ResponsiveContainer width="100%" height={270}>
            <BarChart data={monthly}>
              ...
              <Bar dataKey="total" fill="#1a1a1a" radius={[8,8,0,0]} maxBarSize={34} />
            </BarChart>
          </ResponsiveContainer>
        </Card>

        <Card className="lg:col-span-2 animate-slide-up" style={{animationDelay:'160ms'}}>
          <h3 className="text-base font-black text-ink mb-5">Par catégorie</h3>
          {stats?.byCategory?.length > 0 ? (<>
            <ResponsiveContainer width="100%" height={170}>
              <PieChart><Pie data={stats.byCategory} cx="50%" cy="50%" innerRadius={42} outerRadius={68} paddingAngle={2} dataKey="total" nameKey="category" stroke="#1a1a1a" strokeWidth={2}>
            ...
            <div className="space-y-2 mt-3">{stats.byCategory.map(c => (
              <div key={c.category} className="flex items-center justify-between">
                <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full border-2 border-ink" style={{backgroundColor:expenseCategoryColors[c.category]}} /><span className="text-xs font-semibold text-ink-light">{expenseCategoryLabels[c.category]}</span></div>
                <span className="text-xs font-black text-ink">{formatCurrency(c.total)}</span>
              </div>
            ))}</div>
      ...
      <div className="flex gap-2 flex-wrap">
        {catFilters.map(opt => (
          <button key={opt.value} onClick={() => setFilter(opt.value)}
            className={`px-3 py-1.5 rounded-xl text-xs font-bold border-2 transition-all ${filter===opt.value ? 'nb-nav-active text-ink' : 'border-ink/20 text-ink-light hover:border-ink/40'}`}>
            {opt.label}
          </button>
        ))}
      </div>
      ...
      {exps.length > 0 ? (
        <div className="space-y-3 stagger">{exps.map(exp => (
          <div key={exp.id} className="nb-card-flat p-4 flex items-center justify-between group hover:bg-bg-alt transition-colors">
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 rounded-xl border-2 border-ink flex items-center justify-center flex-shrink-0 shadow-[2px_2px_0_#1a1a1a]"
                style={{backgroundColor: expenseCategoryColors[exp.category]||'#6b7280'}}>
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <Badge variant="lime">{expenseCategoryLabels[exp.category]}</Badge>
                </div>
                <div className="flex gap-2 mt-0.5">
                  {exp.vehicle && <span className="text-[10px] font-semibold text-ink-light">{exp.vehicle.brand} {exp.vehicle.model}</span>}
                  {exp.description && <span className="text-[10px] text-ink-muted">— {exp.description}</span>}
                </div>
              </div>
            </div>
            <div className="flex items-center gap-2 flex-shrink-0 ml-3">
              <span className="text-[10px] font-semibold text-ink-muted">{formatDateShort(exp.date)}</span>
              <button onClick={() => del(exp.id)} className="p-1.5 rounded-lg opacity-0 group-hover:opacity-100 hover:bg-rose/10 text-ink-muted hover:text-rose transition-all">
